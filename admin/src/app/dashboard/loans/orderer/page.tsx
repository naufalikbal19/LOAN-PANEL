...
function FieldBlock({ label, value, mono, full }: { label: string; value: string; mono?: boolean; full?: boolean }) {
  return (
    <div style={full ? { gridColumn: "1 / -1" } : {}}>
      ...
      <p style={{ fontSize: 13, color: value && value !== "—" ? "#ccc" : "#444", fontFamily: mono ? "monospace" : "inherit", wordBreak: "break-word", whiteSpace: "pre-wrap" }}>{value || "—"}</p>
    </div>
  );
}
...
function ReadOnlyField({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div>
      ...
      <div style={{ ...inputStyle, color: "#666", cursor: "not-allowed", userSelect: "none" as const, fontFamily: mono ? "monospace" : "inherit" }}>
        {value}
      </div>
    </div>
  );
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: "#888", fontSize: 13 }}>Senarai permohonan pinjaman pelanggan</p>
        </div>
        <button onClick={() => fetchData(inputSearch, filterStatus)} style={{ background: "#1e1e1e", border: "1px solid #2e2e2e", borderRadius: 8, padding: "8px 14px", color: "#888", cursor: "pointer", display: "flex", alignItems: "center", gap: 6, fontSize: 13, fontFamily: "inherit" }}>
      ...
      <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={10} style={{ textAlign: "center", padding: 44, color: "#555", fontSize: 13 }}>Memuatkan...</td></tr>
              ) : rows.length === 0 ? (
                <tr><td colSpan={10} style={{ textAlign: "center", padding: 44, color: "#555", fontSize: 13 }}>Tiada permohonan dijumpai.</td></tr>
              ) : rows.map((row, i) => (
                <tr key={row.id} style={{ borderBottom: "1px solid #161616", background: i % 2 === 0 ? "transparent" : "#0a0a0a" }}>
                  ...
                  <td style={{ padding: "13px 16px", fontSize: 13, color: "#ccc", fontFamily: "monospace" }}>{row.phone}</td>
                  <td style={{ padding: "13px 16px", fontSize: 13, color: "#888", fontFamily: "monospace" }}>#{String(row.user_id).padStart(4, "0")}</td>
                  <td style={{ padding: "13px 16px", fontSize: 13, color: "#fff", fontWeight: 600 }}>{formatRM(row.amount)}</td>
                  <td style={{ padding: "13px 16px", fontSize: 13, color: "#aaa" }}>{row.loan_terms || <span style={{ color: "#444" }}>—</span>}</td>
                  ...
                  <td style={{ padding: "13px 16px", fontSize: 12, color: "#666", whiteSpace: "nowrap" }}>{formatDate(row.created_at)}</td>
        ...
        <div style={{ padding: "10px 16px", borderTop: "1px solid #1a1a1a" }}>
          <span style={{ fontSize: 12, color: "#555" }}>{rows.length} permohonan</span>
        </div>
      </div>
      ...
      {viewRow && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24 }}>
          <div style={{ background: "#111", border: "1px solid #2e2e2e", borderRadius: 16, width: "100%", maxWidth: 540, maxHeight: "88vh", display: "flex", flexDirection: "column" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px 24px" }}>
                ...
                {(viewRow.sign_url || viewRow.front_ic_url || viewRow.back_ic_url || viewRow.selfie_url) && (
                  <div style={{ gridColumn: "1 / -1" }}>
                    <p style={{ fontSize: 10, color: "#c9a84c", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 12, paddingBottom: 6, borderBottom: "1px solid #1e1e1e" }}>Dokumen</p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                      {[
                        ...
                        <div key={label}>
                          <p style={{ fontSize: 10, color: "#555", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 }}>{label}</p>
                          {url ? (
                            <a href={url} target="_blank" rel="noreferrer" style={{ display: "block" }}>
                              <img src={url} alt={label} style={{ width: "100%", maxHeight: 130, objectFit: "cover", borderRadius: 8, border: "1px solid #2e2e2e", background: "#0c0c0c", display: "block" }}
                                onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                            </a>
                          ) : (
                            <div style={{ height: 80, borderRadius: 8, border: "1px dashed #2e2e2e", display: "flex", alignItems: "center", justifyContent: "center", color: "#444", fontSize: 12 }}>Tiada gambar</div>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  <p style={{ fontSize: 10, color: "#555", fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>Status</p>
      ...
      {editRow && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24 }}>
          <div style={{ background: "#111", border: "1px solid #2e2e2e", borderRadius: 16, width: "100%", maxWidth: 540, maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "18px 24px 14px", borderBottom: "1px solid #1e1e1e", flexShrink: 0 }}>
              <div>
                <h2 style={{ fontSize: 15, fontWeight: 800 }}>Edit Permohonan</h2>
                <p style={{ fontSize: 12, color: "#555", marginTop: 2 }}>#ORD-{String(editRow.id).padStart(5, "0")} · UID #{String(editRow.user_id).padStart(4, "0")}</p>
              </div>
            ...
            <div style={{ overflowY: "auto", padding: "20px 24px", flex: 1 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px 16px" }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  <div style={{ ...inputStyle, color: "#666", cursor: "not-allowed" }}>{formatDate(editRow.created_at)}</div>
                </div>

                {/* Document images */}
                <div style={{ gridColumn: "1 / -1" }}>
                  <p style={{ fontSize: 10, color: "#c9a84c", fontWeight: 700, letterSpacing: 1.5, textTransform: "uppercase", paddingBottom: 6, borderBottom: "1px solid #1e1e1e", marginBottom: 12 }}>Dokumen (URL Gambar)</p>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px 16px" }}>
                    {([
                      ...
                      <div key={label}>
                        ...
                        {val && (
                          <img src={val} alt={label} style={{ marginTop: 6, width: "100%", maxHeight: 80, objectFit: "cover", borderRadius: 6, border: "1px solid #2e2e2e", background: "#0c0c0c" }}
                            onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                        )}
                      </div>
                    ))}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 4 }}>
                    {ALL_STATUSES.map(([key, m]) => (
                      <label key={key} onClick={() => handleStatusChange(key)}
                        style={{ display: "flex", alignItems: "center", gap: 10, padding: "9px 14px", background: editStatus === key ? "rgba(201,168,76,0.06)" : "#0c0c0c", border: `1px solid ${editStatus === key ? "rgba(201,168,76,0.3)" : "#1e1e1e"}`, borderRadius: 9, cursor: "pointer" }}>
                        <div style={{ width: 16, height: 16, borderRadius: "50%", border: `2px solid ${editStatus === key ? "#c9a84c" : "#333"}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  <p style={{ fontSize: 11, color: "#555", marginBottom: 6 }}>Auto-diisi daripada template settings apabila status ditukar. Boleh diedit secara manual.</p>
                  <textarea
      ...
      {deleteRow && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 24 }}>
          <div style={{ background: "#111", border: "1px solid #2e2e2e", borderRadius: 16, padding: 28, width: "100%", maxWidth: 380 }}>
            ...
            <p style={{ fontSize: 13, color: "#888", marginBottom: 12 }}>Tindakan ini tidak boleh dibatalkan.</p>
            <div style={{ background: "#0c0c0c", borderRadius: 9, padding: "12px 14px", marginBottom: 20 }}>
              ...
              <p style={{ fontSize: 12, color: "#888", marginTop: 4 }}>{deleteRow.name} · {formatRM(deleteRow.amount)}</p>
            </div>
