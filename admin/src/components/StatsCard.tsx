...
export default function StatsCard({ label, value, sub, icon, color, trend }: Props) {
  return (
    <div style={{ background: "#161616", border: "1px solid #242424", borderRadius: 16, padding: "20px 22px", display: "flex", flexDirection: "column", gap: 12, position: "relative", overflow: "hidden" }}>
      <div style={{ position: "absolute", top: -20, right: -20, width: 80, height: 80, borderRadius: "50%", background: `${color}10` }} />
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: "#888", letterSpacing: 0.5 }}>{label}</span>
      ...
      <div>
        <p style={{ fontSize: 28, fontWeight: 900, color: "#fff", lineHeight: 1 }}>{value}</p>
      ...
      {trend && (
        <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
          ...
          <span style={{ fontSize: 11, color: "#666" }}>bulan ini</span>
        </div>
      )}
    </div>
  );
